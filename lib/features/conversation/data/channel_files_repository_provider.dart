...
/// Defensive parsing for the channel files API response.
/// Tries `data['files']`, then `data['attachments']`, then bare list.
///
/// Visible for testing.
List<MessageAttachment> parseFileListResponse(Object? data) {
  if (data is List) {
...
class _ApiChannelFilesRepository implements ChannelFilesRepository {
  ...
  Future<List<MessageAttachment>> listFiles(
    ...
    try {
      ...
      return parseFileListResponse(response.data);
    } on AppFailure {
      rethrow;
    } catch (error) {
