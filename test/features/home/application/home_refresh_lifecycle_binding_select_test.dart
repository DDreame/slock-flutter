// =============================================================================
// #613 — homeRefreshRealtimeStateProvider .select() — lifecycle binding
//
// Invariant: INV-REFRESH-REALTIME-SELECT-1
//   homeRefreshRealtimeStateProvider at L26-28 only consumes `status` from
//   RealtimeConnectionState. Mutations to other fields (lastAnyEventAt,
//   lastHeartbeatAt, reconnectAttempts, etc.) must NOT fire downstream.
//
// Strategy:
...
//
// Phase B:
// Replace ref.watch(realtimeServiceProvider) at L28 with
...
void main() {
  ...
  // -------------------------------------------------------------------------
  test(
    'INV-REFRESH-REALTIME-SELECT-1: lastAnyEventAt change does NOT notify '
    'status select',
    () async {
      ...
      store.setLastAnyEventAtDirect(DateTime(2026, 5, 19, 12, 0, 0));

      expect(
        selectNotifyCount,
        0,
        reason: 'lastAnyEventAt change must not notify status select '
            '(INV-REFRESH-REALTIME-SELECT-1)',
      );
  ...
  // -------------------------------------------------------------------------
  test(
    'INV-REFRESH-REALTIME-SELECT-1: lastHeartbeatAt change does NOT notify '
    'status select',
    () async {
      final container = ProviderContainer(
        overrides: [
      ...
      final store = container.read(realtimeServiceProvider.notifier)
          as _ControllableRealtimeService;
      store.setLastHeartbeatAtDirect(DateTime(2026, 5, 19, 12, 0, 0));
  ...
  // -------------------------------------------------------------------------
  test(
    'INV-REFRESH-REALTIME-SELECT-1: status change DOES notify status select',
    () async {
      final container = ProviderContainer(
        overrides: [
          realtimeServiceProvider
